...
static int __init qnap_tsx51_led_init(void) {
	int ret;
	if (ret = request_module("gpio_f7188x"))
		return ret;
...
static void __exit qnap_tsx51_led_exit(void) {
	int i;
	for (i = 0; i < ARRAY_SIZE(qnap_tsx51_devs); i++)
		platform_device_unregister(qnap_tsx51_devs[i]);
}

module_init(qnap_tsx51_led_init);
module_exit(qnap_tsx51_led_exit);
